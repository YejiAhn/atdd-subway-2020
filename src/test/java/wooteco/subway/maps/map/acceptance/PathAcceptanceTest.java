...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    @DisplayName("노선별 추가 요금이 있는 노선이 있는 경로를 조회한다.")
    @Test
    void findPathWithAdditionalFare() {
        //when
        ExtractableResponse<Response> response = 거리_경로_조회_요청("DURATION", 강남역, 양재역);

        //then
        적절한_경로를_응답(response, Lists.newArrayList(강남역, 양재역));
        총_거리와_소요_시간을_함께_응답함(response, 2, 1, 2150);
    }
